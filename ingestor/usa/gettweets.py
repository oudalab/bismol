...
FILENAME = environ.get('TWEET_FILE') or \
        '/data/tweetsdb/tweet_usa_{}.json.gz'.format(time.strftime("%Y%m%d%H%M%S"))
...
class USAStreamListener(tweepy.StreamListener):
    """Extended Steam listener for these food tweets."""

    # def __init__(self,api=None):
    def __init__(self, api):
        super(USAStreamListener, self).__init__(api)  # Python 3
...
def start(args):
    ...
    mystreamlistener = USAStreamListener(api)
